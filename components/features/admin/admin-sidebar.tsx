...
export const ADMIN_NAV_ITEMS: NavItem[] = [
  ...
  { id: "statistics", label: "Score Analytics", icon: LineChart },
  { id: "exports", label: "Data Exports & Hub", icon: Download, badge: "Hub" },
  { id: "academic-archives", label: "New Year Archive", icon: Archive, badge: "New" },
  { id: "system", label: "System & Settings", icon: Database },
]

export const STATS_ALLOWED_TAB_IDS = ["tracking", "statistics", "exports"]
...
export function AdminSidebar({ activeTab, onSelectTab, userRole }: AdminSidebarProps) {
  ...
  const isStats = userRole === "stats"

  const visibleNavItems = ADMIN_NAV_ITEMS.filter((item) => {
    if (isStats) {
      return STATS_ALLOWED_TAB_IDS.includes(item.id)
    }
    if (item.superAdminOnly && userRole !== "super_admin") return false
    return true
  })

  // The most frequent destinations stay one tap away on phones.
  // Stats accounts have exactly 3 tabs, displaying all 3 directly.
  const mobilePrimaryItems = isStats
    ? visibleNavItems
    : visibleNavItems.filter((item) => ["classrooms", "users", "tracking"].includes(item.id))
  const mobileMoreItems = isStats
    ? []
    : visibleNavItems.filter((item) => !mobilePrimaryItems.some((primary) => primary.id === item.id))
        ...
        <div className="sticky top-20 bg-card/60 backdrop-blur-md border border-border/80 rounded-2xl p-3 shadow-xs space-y-4">
          <div className="px-3 py-1">
            <p className="text-[10px] uppercase tracking-wider font-bold text-muted-foreground">
              {isStats ? "Stats & Analytics Portal" : "Management Portal"}
            </p>
          </div>
