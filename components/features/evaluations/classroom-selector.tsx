          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            {filteredClassrooms.map((classroom, index) => (
              ...
              >
                <Button
                  variant="outline"
                  className="w-full min-h-16 h-auto p-4 sm:p-5 flex flex-col items-start hover:bg-primary/10 hover:border-primary/70 transition-[background-color,border-color,color,box-shadow,opacity,transform] bg-card text-left whitespace-normal shadow-xs hover:shadow-md group rounded-2xl cursor-pointer"
                  onClick={() => onSelect(classroom)}
                >
                  <div className="flex items-center gap-2 w-full mb-2">
                    <m.div
                      className="w-2 h-2 rounded-full bg-primary opacity-0 group-hover:opacity-100 shrink-0"
